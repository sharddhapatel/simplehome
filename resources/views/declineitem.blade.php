...
<div class="main-container">
    ...
    <h3 class="col-12 text-center tm-section-title">Decline Items</h3>
  </header>
        ...
        <tr>
           ...
           <td style="width:10%" class="add-img-td"><strong>Rs.</strong>{{$image->price}}</td>
            ...
            <td style="width:8%" class="action-td">
                <div>
                    <a href ='updatedeclineitem/{{ $image->id }}'><button type="submit" class="btn btn-success btn-sm">Edit</button></a>
                   
                    <a href ='deletedeclineitem/{{ $image->id }}'><button type="submit" class="btn btn-danger btn-sm">Delete</button></a>
                </div>
            </td>
             @endforeach
               @if(count($data)<=0)
